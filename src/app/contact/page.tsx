...
import { Mail, Phone, MapPin, Send, ChevronRight, ChevronDown, HelpCircle } from 'lucide-react'
...
export default function ContactPage() {
  ...
  const toggleFaqItem = (id: string) => {
    setOpenFaqItems(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id])
  }
      ...
      <section className="py-24" style={{ backgroundColor: '#FFFFFF' }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ color: '#1E2333' }}>
              Frequently Asked Questions
            </h2>
            <p className="text-lg" style={{ color: '#676D82' }}>
          ...
          {faqs.map((section, sIdx) => (
            <div key={section.category} className="mb-10">
              <div className="flex items-center gap-4 mb-5">
                <span className="text-base font-bold" style={{ color: '#65B863' }}>
                  {String(sIdx + 1).padStart(2, '0')}
                </span>
                <span className="w-12 h-px" style={{ backgroundColor: '#65B863' }}></span>
                <h3 className="text-xl font-bold" style={{ color: '#1E2333' }}>{section.category}</h3>
              </div>

              <div className="space-y-3">
                {section.questions.map((faq, qIdx) => {
                  const id = `${sIdx}-${qIdx}`
                  const isOpen = openFaqItems.includes(id)
                  return (
                    <div key={id} className="rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: '#F7F9FA', border: '1px solid #EAEDE6' }}>
                      <button
                        onClick={() => toggleFaqItem(id)}
                        className="w-full flex items-center justify-between p-5 text-left transition-all hover:bg-gray-100"
                      >
                        <div className="flex items-center gap-3">
                          <HelpCircle size={18} style={{ color: '#65B863', flexShrink: 0 }} />
